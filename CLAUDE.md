...
| -------------- | --------------- | ------------------------------------------------------------------------ |
| `API_BASE_URL` | (auto-detected) | hifi-api instance URL (optional - uses DEFAULT_API_PROVIDERS if not set) |
...
**Currently Working (as of March 2026):**

- `https://api.monochrome.tf` (primary)
- `https://triton.squid.wtf` (secondary)

**Broken (return 403 "Upstream API error" on /track/):**
...
**Note:** The app automatically falls back to working providers. Proxy errors (403 with "Upstream API error") trigger fallback to the next provider.
